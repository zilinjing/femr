...
def map_statistics(
    ...
    *,
    ...
    for subject in tqdm(subjects):
        ...
        # print(f"subject events are: {subject.events}")
...
# the statistics example


def _worker_map_statistics(
    db_path: str,
    id_chunk: List[int],
    num_subjects: int,
    ontology: femr.ontology.Ontology,
    ...
    """
    Re-open the SubjectDatabase inside each process, then call map_statistics.
    """
    subdb = meds_reader.SubjectDatabase(db_path, num_threads=1)
    subjects = (subdb[sid] for sid in id_chunk)
...
class HierarchicalTokenizer(transformers.utils.PushToHubMixin):
    ...
    def train(
        ...
        - Splits the db into chunks of `chunk_size` subjects
        - Uses a Pool of `n_workers` processes to run `map_statistics` on each chunk
        - Reduces (merges) all chunk‐statistics with `agg_statistics`
        """

        # 1) Prepare
        db = meds_reader.SubjectDatabase(db_path, num_threads=1)
        subject_ids = list(db)                # these are your real IDs (e.g. 10000032, …)
        num_subjects = len(subject_ids)

        properties = db.properties
        for b in banned_properties:
            properties.pop(b, None)

        # 3) Break the full ID list into chunks
        chunks = [
       ...
       # 4) Parallel map + reduce
        if n_workers == 1:
            ...
            with multiprocessing.Pool(processes=n_workers) as pool:
                args = [
                    (db_path, chunk, num_subjects, ontology, properties)
                    for chunk in chunks
                ]
                chunk_stats = pool.starmap(_worker_map_statistics, args)

            # merge all chunk‐statistics
            statistics = chunk_stats[0]
            for st in chunk_stats[1:]:
                statistics = agg_statistics(statistics, st)


        # 4) Convert to final vocab & instantiate
        whatever = convert_statistics_to_msgpack(statistics, vocab_size, num_numeric, ontology, min_fraction)
        # return cls(dict_msgpack, ontology)
